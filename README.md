...
If you don't care about the aspect ration and want an exact dimension, you can use `!`:

``` erb
<%= link_to "Image", attachment_url(@user, :profile_image, :limit, 400, 1000!) %>
```

keep in mind that It's also important to remember you can not scretch the image, even you set
a larger width or height the image will keep its default dimensions. For example: if you set a
